appExpled.lazyController('gallery', function ($scope, $rootScope, $http, $interval,$location) {
            ...
            $http({
                    ...
                    for(var j=0;j<$scope.cantFilas;j++){
                        ...
                        $scope.procesoBase.filas.push({
                            index:j+""+j,
                            Pantalla1:[
                                {nombre:$scope.PantCentro[au],salida:(j*2)+1,ipImpresora:"\\DESKTOP-5IJUQ6F\ZDesigner",PanZPL:0,linea:0,fila:j,pantalla:'Pantalla1',etiqueta:$scope.etiquetaBase,Stock:-1,zplBase:"",aux:0},
                                {nombre:$scope.PantCentro[au],salida:(j*2)+1,ipImpresora:"191.089."+j+1,PanZPL:1,linea:0,fila:j,pantalla:'Pantalla1',etiqueta:$scope.etiquetaBase,Stock:-1,zplBase:"",aux:0},
                                {nombre:$scope.PantCentro[au],salida:(j*2)+1,ipImpresora:"191.089."+j+1,PanZPL:2,linea:0,fila:j,pantalla:'Pantalla1',etiqueta:$scope.etiquetaBase,Stock:-1,zplBase:"",aux:0},
                                {nombre:$scope.PantCentro[au],salida:(j*2)+1,ipImpresora:"191.089."+j+1,PanZPL:3,linea:0,fila:j,pantalla:'Pantalla1',etiqueta:$scope.etiquetaBase,Stock:-1,zplBase:"",aux:0}
                            ],
                            Pantalla2:[
                                {nombre:$scope.PantCentro[au+1],salida:(j*2)+2,ipImpresora:"191.089."+j+1,PanZPL:0,linea:0,fila:j,pantalla:'Pantalla2',etiqueta:$scope.etiquetaBase,Stock:-1,zplBase:"",aux:0},
                                {nombre:$scope.PantCentro[au+1],salida:(j*2)+2,ipImpresora:"191.089."+j+1,PanZPL:1,linea:0,fila:j,pantalla:'Pantalla2',etiqueta:$scope.etiquetaBase,Stock:-1,zplBase:"",aux:0},
                                {nombre:$scope.PantCentro[au+1],salida:(j*2)+2,ipImpresora:"191.089."+j+1,PanZPL:2,linea:0,fila:j,pantalla:'Pantalla2',etiqueta:$scope.etiquetaBase,Stock:-1,zplBase:"",aux:0},
                                {nombre:$scope.PantCentro[au+1],salida:(j*2)+2,ipImpresora:"191.089."+j+1,PanZPL:3,linea:0,fila:j,pantalla:'Pantalla2',etiqueta:$scope.etiquetaBase,Stock:-1,zplBase:"",aux:0}
                            ]
                        })
                        au = au +2;
                    }
        ...
        $http({
            ...
            angular.forEach(data.MATERIALES,function(val,key){
                if(val.ZMAT_ESPECIE == especie){
                    $scope.materialesTerminado.push({'DESCRIPTION':val.MATNR,VALUE_CHAR: val.MATNR, 'KILOS':val.NTGEW});
                }
            });
        });
    }
    ...
    $scope.validate = function(){
        console.log($scope.etiqueta.etiqueta);
            var zplBase =  $scope.etiqueta.zplBase;
             ...
             console.log(dataEtiqueta);
                 if(dataEtiqueta.length == 0){
                  ...
                  if(obj.etiqueta.codmat == ""){
                      ...
                      var datos = 'z='+obj.id+'&zpl='+obj.zpl+'&s='+obj.Stock+'&p='+id+'&pr='+obj.proceso+'&k='+$scope.etiqueta.etiqueta.codmat.KILOS+'&consul=insert'+'&l='+obj.linea+'&iz='+obj.ipImpresora
    ...
    $scope.AgregarProceso = function(item){
        ...
        if(item.codigo === "" && item.procesos.length<3){
            ...
            if(auxtrue){
                        ...
                        .success(function(data){
                            ...
                            if(item.procesos[item.procesosIndex].kilosEtiqueta >= item.procesos[item.procesosIndex].kilosGranel){
                               // item.procesos[item.procesosIndex].ESTADO = "TERMINADO";
                                     $http({
                                        method: 'POST',
                                        url: IPSERVER+'/JSON_ZMOV_INS_ETI.aspx?c='+$rootScope.userData.centro+'&p='+angular.uppercase($scope.code)+'&e=TERMINADO&con=Modific',
                                        contentType: 'application/json; charset=utf-8',
                                        dataType: 'json',
                                        timeout:500000
                ...
                $http({
                     method: 'POST',
                     url: IPSERVER+'/JSON_ZMOV_INS_ETI.aspx?c='+$rootScope.userData.centro+'&p='+angular.uppercase(item.codigo)+'&e=VIGENTE&con=Insert',
                     contentType: 'application/json; charset=utf-8',
                     dataType: 'json',
                     timeout:500000
                 }).success(function(datoss){
