...
appExpled.lazyController('vista', function ($scope,$http, $rootScope) {
     $scope.name=  appConfig.ZPL.nombre;
       ...
       $scope.Envio = function(){
          $.ajax({
              type:"POST",
              url:"http://ddcprd.expled.cl/ddc/JSON_ZPL_SAVE.aspx?key="+encodeURI(appConfig.ZPL.ZPL)+"&w="+appConfig.ZPL.width+"&h="+appConfig.ZPL.height+"&d="+appConfig.ZPL.dpmm,
              dataType:"text",
              success:function(dato){ 
                   ...
                   $http({
                    method: 'POST',
                    url: 'http://ddcprd.expled.cl/ddc/JSON_DB_ZPL.aspx?z='+appConfig.ZPL.ZPL+'&d='+appConfig.ZPL.dpmm+'&t='+appConfig.DatosZPL.Tamanio+'&i='+appConfig.DatosZPL.IpZebra+'&r='+appConfig.DatosZPL.Ruta+'&n='+appConfig.ZPL.nombre,
