appExpled.lazyController('ctrExportacion', function ($scope, $routeParams, $rootScope,$http){
    ...
    $scope.controlDescarte = function(){
        ...
        $http({
            ...
            angular.forEach(data.LT_LIST_MAT,function(val,key){
                $scope.listarListaMateriales.push({DESCRIPTION:val.STLAL+' - '+val.STKTX,VALUE_CHAR:val.STLAL});
            })
            console.log(data);
    ...
    angular.forEach($rootScope.COLOR , function(value,key){
        if(value.ATBEZ==$rootScope.dataSeleccion.especie.VALUE_CHAR)
            $scope.listarColor.push({DESCRIPTION:value.DESCRIPTION ,VALUE_CHAR:value.VALUE_CHAR});
    })
    //$rootScope.PLU
    $scope.listarPLU =[];
    ...
    $scope.validaFormRecep = function (){
           var jsonValidate = [
                ...
                {campo:"Categoria",value:$rootScope.datosPaletizaje.detalle[$rootScope.idxTab].categoria,type:"aSelect",index:"VALUE_CHAR"},
        ...
        if($rootScope.userData.mail=="servicio"){
            jsonValidate.push({campo:"IDG",value:$rootScope.datosPaletizaje.detalle[$rootScope.idxTab].cliente,type:"aSelect",index:"VALUE_CHAR"});
        }
        if(!$rootScope.validaForm(jsonValidate))return 0;
    ...
    $scope.generaXML = function () {
      if($rootScope.userData.mail=="servicio"){
        ...
        if(1==1){
            ...
            cadenaXML += '               <tem:MATERIAL>'+angular.uppercase($rootScope.datosPaletizaje.detalle[0].material.MATNR)+'</tem:MATERIAL>';
            cadenaXML += '               <tem:BATCH></tem:BATCH>';
            cadenaXML += '               <tem:CHARACT>Z'+$rootScope.dataSeleccion.especie.VALUE_CHAR+'_TIPIFICACION</tem:CHARACT>';
