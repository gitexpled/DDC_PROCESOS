appExpled.lazyController('ctrRePaletizar', function ($scope, $routeParams, $rootScope,$http) {
    ...
    $scope.Pop_Up_Material = function () {
        ...
        $http({
            method: 'POST',
            url: IPSERVER + 'JSON_ZMOV_10020.aspx?IV_MATNR=' + $scope.LT_HU_CABECERA[0].MATNR + '&IV_WERKS=' + angular.uppercase($rootScope.userData.centro) + '&IV_HU_GRP4=' + $scope.alturaDestino.VEGR4 + '&IV_TIP_PACKING=' + (($rootScope.userData.mail === 'recepcionPallet') ? 'S' : 'C'),
                           ...
                           <tem:ZMOV_50001>\
                              <tem:datos>\
                                <tem:IS_HEADER_HU_DEST>\
                                  ...
                                  <tem:HU_EXID>'+$scope.folioDestino+'</tem:HU_EXID>\
                                  ...
                                  <tem:LGORT_DS>'+(($scope.LT_HU_CABECERA.length==0)?$scope.LT_HU_CABECERA[0].LGORT:'')+'</tem:LGORT_DS>\
                                 ...
                                 <tem:IV_EXIDV_DEST>'+$scope.folioDestino+'</tem:IV_EXIDV_DEST>\
                                 <tem:LT_ITEM_UNPACK>';
        angular.forEach($scope.LT_HU_POSICION,function(val,key){
            if(val.eliminaCaja){
